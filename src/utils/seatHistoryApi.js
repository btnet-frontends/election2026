...
const HISTORY_API_URL = 'https://doqvf81n9htmm.cloudfront.net/files/2026election/getParliamentaryHistory';
...
export async function fetchSeatHistory() {
  try {
    const response = await fetch(`${HISTORY_API_URL}?v=${Date.now()}`);
    if (!response.ok) {
